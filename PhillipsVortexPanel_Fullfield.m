...
% Pt coords
Ncoords = 40;       % Number of AF coordinates. Num of ctrl pts = N*2 -2
...
Gamma = (0.5*(gamma(1:end-1)+gamma(2:end))).*s;     % Circulation strength of each panel (integrated)

fieldpt = [1 0.005];


for i=1:length(Gamma)
    dxf(i) = ctrlp(i,1) - fieldpt(1);
    dyf(i) = ctrlp(i,2) - fieldpt(2);
    rf(i) = sqrt(dxf(i)^2 + dyf(i)^2);            % Magnitude of r vector
    angf(i) = atan2(dyf(i), dxf(i));                       % Angle of r vector 
    vang(i,:) = [-dyf(i) dxf(i)]./rf(i);              % Unit direction of induced vel (cross of gamma with r vector) 
    vi(i,:) = Gamma(i)/(2*pi*rf(i))*vang(i,:);          % Biot-Savart for induced vel magnitude
...
quiver(fieldpt(:,1),fieldpt(:,2),Viscaled(:,1),Viscaled(:,2),'Color','b')
hold off
